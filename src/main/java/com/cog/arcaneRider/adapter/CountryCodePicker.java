...
//        Log.d(TAG, "Initialization of CCP");
        ...
        holderView = mInflater.inflate(com.cog.arcaneRider.R.layout.layout_code_picker, this, true);
        textView_selectedCountry = (TextView) holderView.findViewById(com.cog.arcaneRider.R.id.textView_selectedCountry);
        relative_holder = (RelativeLayout) holderView.findViewById(com.cog.arcaneRider.R.id.relative_countryCodeHolder);
        imageViewArrow = (ImageView) holderView.findViewById(com.cog.arcaneRider.R.id.imageView_arrow);
        codePicker = this;
...
//        Log.d(TAG, "Applying custom property");
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                com.cog.arcaneRider.R.styleable.CountryCodePicker,
                0, 0);
        //default country code
        try {
            //hide nameCode. If someone wants only phone code to avoid name collision for same country phone code.
            hideNameCode = a.getBoolean(com.cog.arcaneRider.R.styleable.CountryCodePicker_hideNameCode, false);

            //autopop keyboard
            setKeyboardAutoPopOnSearch(a.getBoolean(com.cog.arcaneRider.R.styleable.CountryCodePicker_keyboardAutoPopOnSearch, true));

            //if custom language is specified, then set it as custom
            int attrLanguage = LANGUAGE_ENGLISH;
            if (a.hasValue(com.cog.arcaneRider.R.styleable.CountryCodePicker_ccpLanguage)) {
                attrLanguage = a.getInt(com.cog.arcaneRider.R.styleable.CountryCodePicker_ccpLanguage, 1);
            }
            customLanguage = getLanguageEnum(attrLanguage);

            //custom master list
            customMasterCountries = a.getString(com.cog.arcaneRider.R.styleable.CountryCodePicker_customMasterCountries);
            refreshCustomMasterList();

            //preference
            countryPreference = a.getString(com.cog.arcaneRider.R.styleable.CountryCodePicker_countryPreference);
            refreshPreferredCountries();

            //default country
            defaultCountryNameCode = a.getString(com.cog.arcaneRider.R.styleable.CountryCodePicker_defaultNameCode);
            ...
            if (!setUsingNameCode) {
                int defaultCountryCode = a.getInteger(com.cog.arcaneRider.R.styleable.CountryCodePicker_defaultCode, -1);
            ...
            int contentColor;
            if (isInEditMode()) {
                contentColor = a.getColor(com.cog.arcaneRider.R.styleable.CountryCodePicker_contentColor, 0);
            } else {
      ...
      //      }
            ...
            //text size
            int textSize = a.getDimensionPixelSize(com.cog.arcaneRider.R.styleable.CountryCodePicker_textSize, 0);
            if (textSize > 0) {
            ...
            int arrowSize = a.getDimensionPixelSize(com.cog.arcaneRider.R.styleable.CountryCodePicker_arrowSize, 0);
            if (arrowSize > 0) {
